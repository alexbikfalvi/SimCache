...
CModelCacheLfu2::CModelCacheLfu2(
	...
	)
{
	...
	// Cache
	this->cache = new TModelCacheLfuPeer2[this->workload->NumPeers()];
	assert(this->cache);
...
CIndication CModelCacheLfu2::SegmentRequest(
	__time			timestamp,
	unsigned int	peer,
	unsigned int	video,
	CBitmap*		bitmap
	)
{
	...
	for(TModelCacheLfuPeer2::iterator iter = this->cache[peer].begin(); iter != this->cache[peer].end(); iter++)
...
void CModelCacheLfu2::SegmentResponse(
	unsigned int		peer,
	unsigned int		video,
...
void CModelCacheLfu2::ClearReceiver(
	unsigned int		peer
	)
{
...
unsigned char CModelCacheLfu2::Add(
	__time				timestamp,
	unsigned int		peer,
	unsigned int		video,
	...
	)
{
	...
	if(this->cache[peer].size() < this->cacheSize)
	{
		...
		this->cache[peer].insert(pair<unsigned __int64, SModelCacheLfuEntry>(0, entry));
	...
	)
{
	for(TModelCacheLfuPeer2::iterator iter = this->cache[peer].begin(); iter != this->cache[peer].end(); iter++)
		if((iter->second.video == video) && (iter->second.segment == segment))
		{
			...
			// Add new entry
			this->cache[peer].insert(pair<unsigned __int64, SModelCacheLfuEntry>(entry.counter, entry));
...
TModelCacheVideo* CModelCacheLfu2::GetPeers(
	unsigned int		video,
	unsigned int		segment
	)
{
